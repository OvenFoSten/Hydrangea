...
A Context Area is a caller-defined object that contributes generic `Message` objects to a `CoopContext`. It also exposes its scheduling and lifetime state so that `CoopContext` can decide when to advance it again and when its affected Context tail may be reclaimed.
...
| --- | --- | --- |
| `life_state` | During collection and before advancing | Exposes whether the Area remains active or has retired. |
| `flow_state` | After every successful `advance()` | Chooses whether this Area keeps the cursor or yields to the next Area. |
| `observe(context)` | Immediately before a repeated advance | Observes a shallow, read-only snapshot of the Area's current EffectRange. |
| `advance()` | When a retained Area reaches the cursor | Advances the Area once and returns generic, caller-constructed messages. |
...
- `retain` means that the Area may still observe and advance.
- `retired` means that the Area will no longer advance and is eligible for collection.
...
- `exclusive` keeps the cursor on the current Area and returns the Context immediately after its advance.
...
3. A previous non-empty `advance()` has already created an EffectRange.
...
`observe()` may change the Area to `retired`. If it does, `CoopContext` skips that Area's `advance()` for the current pass.

### `advance()`

`advance()` advances the Area once and returns `list[Message]`, not provider-native content. Hydrangea converts each message through the active provider implementation and appends it with `Context.emplace_message()`.

- A non-empty result creates or extends the Area's EffectRange.
- An empty result does not create or update an EffectRange.
- The first non-empty advance fixes `earliest`.
- Later non-empty advances move `latest` to the last newly appended message.
...
flowchart TD
    ...
    accDescr: CoopContext repeatedly observes and advances a retained Area, then waits until a retired Area can be promoted and reclaimed.
    ...
    Observe --> AreaAdvance["CoopContext calls Area.advance()<br/>if Area remains retained"]
    AreaAdvance --> Messages["Area returns list[Message]"]
...
- Change scheduling and lifetime state from `observe()` or `advance()`, not from GC callbacks.
